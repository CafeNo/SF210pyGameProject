...
class Enemy(pygame.sprite.Sprite):
    ...
    *** Group Hooked should use in life cycle of game for update thing all sprite only (if use in all behave will raise performace issue.)***
    ...
    def __init__(self, pos_x: int, pos_y: int, screen: pygame.Surface, hp: int, enemyImage: str) -> None:
        super().__init__()

        self.screen = screen
        self.screen_rect = self.screen.get_rect()
        self.sprite = pygame.image.load(enemyImage)
        self.hpImage = enemyImage
        if enemyImage.find("characterModel") > -1:
            self.sprite = pygame. transform. scale(
                self.sprite, (self.sprite.get_width()/6, self.sprite.get_height()/6))
            # self.image = pygame.Surface(
            #     (self.sprite.get_width(), self.sprite.get_height()))
        else:
            self.sprite = pygame. transform. scale(
                self.sprite, (self.sprite.get_width(), self.sprite.get_height()))
        ...
        self.playerHP = pygame.image.load("./assets/pic_enemy_hp_bar.png")
    ...
    def update(self):
        ...
        if (time_now > self.pace_time + self.speed):   # is it time to move again
            ...
            self.rect.x += self.direction[0] * \
                self.pace_size     # Move some pixels
    ...
    def drawHpbar(self) -> None:
        ...
        a = pygame.transform.scale(
            self.playerHP, (self.image.get_width() * (self.hp / self.maxHp), 9))
